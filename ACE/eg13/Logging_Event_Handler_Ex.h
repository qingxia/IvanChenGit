...
{
private:
    // Time when a client last send a log record.
...
public:
    ...
    Logging_Event_Handler_Ex(ACE_Reactor* reactor, const ACE_Time_Value& max_client_timeout = ACE_Time_Value(MAX_CLIENT_TIMEOUT))
    ...
    // Called when a timout expires to check if the client has
